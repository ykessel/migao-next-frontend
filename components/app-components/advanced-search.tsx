...
export default function AdvancedPropertySearch() {
  ...
  const addPriceFilter = () => {
    ...
    const filteredFilters = activeFilters.filter(
      (f) => !(f.field === "rentPricePerMonth" && f.type === "RANGE")
    );
  ...
  const addDateFilter = () => {
    ...
    const filteredFilters = activeFilters.filter(
      (f) => !(f.field === "isAvailableForRentalFrom" && f.type === "RANGE")
    );
    setActiveFilters([...filteredFilters, filter]);
  };
  ...
  const addLocationFilter = () => {
    ...
    const filteredFilters = activeFilters.filter(
      (f) => !(f.field === "location" && f.type === "GEO_DISTANCE")
    );
    setActiveFilters([...filteredFilters, filter]);
  };
  ...
  const addPropertyTypeFilter = () => {
    ...
    const filter = builder
      .term("propertyType", propertyType, "string")
      .build()[0];

    // Remove existing property type filters
    const filteredFilters = activeFilters.filter(
      (f) => !(f.field === "propertyType" && f.type === "TERM")
    );
    setActiveFilters([...filteredFilters, filter]);
  };
  ...
  const addRoomsFilter = () => {
    ...
    const filter = builder
      .numberRange(
        "rooms",
        rooms.min ? Number(rooms.min) : undefined,
        rooms.max ? Number(rooms.max) : undefined
      )
      .build()[0];

    // Remove existing rooms filters
    const filteredFilters = activeFilters.filter(
      (f) => !(f.field === "rooms" && f.type === "RANGE")
    );
    setActiveFilters([...filteredFilters, filter]);
  };
  ...
  const getFilterDisplayText = (filter: PropertyFilter): string => {
    switch (filter.type) {
      case "RANGE":
        if (
          filter.field === "rentPricePerMonth" ||
          filter.field === "rooms" ||
          filter.field === "isAvailableForRentalFrom"
        ) {
          const value =
            filter.value as import("@/types/filter").RangeFilterValue;
          ...
          if (filter.field === "isAvailableForRentalFrom") {
            return `Disponible: ${
              value.gte ? new Date(value.gte).toLocaleDateString() : ""
            } - ${value.lte ? new Date(value.lte).toLocaleDateString() : ""}`;
          }
        }
        break;
      case "TERM":
        if (filter.field === "propertyType") {
          const value =
            filter.value as import("@/types/filter").TermFilterValue;
          return `Tipo: ${value.value}`;
        }
        break;
      case "GEO_DISTANCE": {
        const value =
          filter.value as import("@/types/filter").GeoDistanceFilterValue;
              ...
              <CardContent className="space-y-2">
                <Select value={propertyType} onValueChange={setPropertyType}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="CASA_FLOTANTE_EMBARCACION">
                      Casa Flotante
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <CardContent className="space-y-2">
                <div className="flex gap-2">
                  <Input
                    ...
                    value={rooms.min}
                    onChange={(e) =>
                      setRooms((prev) => ({ ...prev, min: e.target.value }))
                    }
                  />
                  <Input
                    type="number"
                    ...
                    value={rooms.max}
                    onChange={(e) =>
                      setRooms((prev) => ({ ...prev, max: e.target.value }))
                    }
                  />
                </div>
          ...
          {searchResults && (
            <div className="space-y-4">
              <h3 className="font-semibold">
                Encontradas {searchResults.total} propiedades (Página{" "}
                {searchResults.page} de {searchResults.total / 9})
              </h3>
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-green-600">
                            ${property.rentPricePerMonth}
                          </p>
                          <p className="text-sm text-gray-500">
                            {property.currency}/mes
                          </p>
                        </div>
                      </div>
                    </CardContent>
